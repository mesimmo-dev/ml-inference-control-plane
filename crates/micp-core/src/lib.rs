//! Shared domain types for the ML inference control plane.
//!
//! Scoring and feasibility checks are pure functions so the same math
//! can run in the API process, in simulation, and behind the WASM ABI.
...
mod tests {
    ...
    fn quality_heavy_weights_can_prefer_slower_model() {
        ...
        // Lift the latency filter so both are feasible; scoring should
        // then follow the quality weight.
        let demand = 5.0;
